...
const dataset = { id: "ds-1", name: "My measures" };

afterEach(() => {
  cleanup();
});

describe("measures DataDeleteConfirmation", () => {
  ...
  it("renders the dialog with the dataset name and wires the action buttons", () => {
    ...
    expect(screen.getByText("settings_measures_delete_body:My measures")).toBeInTheDocument();

    fireEvent.click(screen.getByText("cancel"));
    expect(onCancel).toHaveBeenCalledTimes(1);

    fireEvent.click(screen.getByText("settings_measures_delete_action"));
  ...
  it("disables the confirm button while a delete is in progress", () => {
    ...
    const confirmBtn = screen.getByText("settings_measures_delete_action").closest("button");
