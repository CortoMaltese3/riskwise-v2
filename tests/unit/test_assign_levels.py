"""Unit tests for ``BaseHandler.assign_levels``.

The method buckets numeric values from a DataFrame column into ordinal
...
uses for hazard vs. impact layers. The logic previously lived on
``HazardHandler`` and ``ImpactHandler``; it was hoisted to ``BaseHandler``
when the two copies were deduplicated.
"""
...
class TestAssignLevelsAscending:
    def test_values_are_bucketed_by_ascending_thresholds(self, handler: BaseHandler) -> None:
    ...
    def test_value_at_bucket_boundary_falls_into_upper_bucket(self, handler: BaseHandler) -> None:
        df = pd.DataFrame({"rp10": [20]})
        result = handler.assign_levels(df, {"rp10": [10, 20, 30, 40]})
        assert list(result["rp10_level"]) == [2]

    def test_value_below_first_threshold_pins_to_level_one(self, handler: BaseHandler) -> None:
        df = pd.DataFrame({"rp25": [-100, 0, 9]})
        result = handler.assign_levels(df, {"rp25": [10, 20]})
        assert list(result["rp25_level"]) == [1, 1, 1]

    def test_value_at_or_above_last_threshold_pins_to_max_level(self, handler: BaseHandler) -> None:
...
class TestAssignLevelsDescending:
    def test_values_are_bucketed_by_descending_thresholds(self, handler: BaseHandler) -> None:
    ...
    def test_value_at_or_below_last_threshold_pins_to_max_level(self, handler: BaseHandler) -> None:
...
class TestAssignLevelsMultipleReturnPeriods:
    def test_each_return_period_gets_its_own_level_column(self, handler: BaseHandler) -> None:
