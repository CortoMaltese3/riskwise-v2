...
The remaining v1 handler test (``backend/report/test_report_handler.py``)
is excluded — it is scope-bounded for deletion in a follow-up issue and
would otherwise block CI. It is also already excluded from pytest
collection via ``pyproject.toml``. The impact/exposure/hazard equivalents
were deleted in issue #167.
...
# Remaining legacy v1 handler tests — ignored by pytest, scoped for deletion.
LEGACY_TEST_PATTERNS = ("backend/report/test_report_handler.py",)
